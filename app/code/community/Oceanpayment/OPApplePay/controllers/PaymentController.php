 ...
 * @package 	Oceanpayment_ApplePay
 * @copyright	Copyright (c) 2009 Oceanpayment,LLC. (http://www.oceanpayment.com)
 */
class Oceanpayment_OPApplePay_PaymentController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * When a customer chooses ApplePay on Checkout/Payment page
	 *
	 */
	public function redirectAction()
	{
		$session = Mage::getSingleton('checkout/session');
		$session->setApplePayPaymentQuoteId($session->getQuoteId());
		...
		$order->getStatus(),
		Mage::helper('opapplepay')->__('Customer was redirected to applepay')
		);
		$order->save();
		...
		//获取配置的pay mode
		$standard = Mage::getModel('opapplepay/payment');
		...
		if($pay_mode == 1){
			...
			$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opapplepay/redirect'));
			$this->renderLayout();	
		}else{
			$this->getResponse()
					->setBody($this->getLayout()
					->createBlock('opapplepay/redirect')
					->setOrder($order)
					->toHtml());
		}
	...
	public function noticeAction()
	{
		...
		$model = Mage::getModel('opapplepay/payment');
		$this->_order = Mage::getModel('sales/order');
		...
		if($_REQUEST['response_type'] == 1){
			...
			switch($this->_validated()){
				case 1:
					...
					Mage::helper('opapplepay')->__(self::PUSH.$authType.'Payment Success!'.$history), true);
				...
				case 0:
					...
					Mage::helper('opapplepay')->__(self::PUSH.$authType.'Payment Failed!'.$history));	
				...
				case -1:
					...
					Mage::helper('opapplepay')->__(self::PUSH.'(Pre-auth)Payment Pending!'.$history));
				...
				case '10000':
					...
					Mage::helper('opapplepay')->__(self::PUSH.'(High Risk)Payment Failed!'.$history));
	...
	public function returnAction()
	{
		...
		switch($this->_validated()){
			case 1:
				...
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_accepted'),
				Mage::helper('opapplepay')->__(self::BrowserReturn.'Payment Success!'.$history), true);	
				
				$order->save();
				$url = 'opapplepay/payment/success';
				break;
			case 0:
				//支付失败
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_refused'),
				Mage::helper('opapplepay')->__(self::BrowserReturn.'Payment Failed!'.$history));					
				
				$order->save();
				$url = 'opapplepay/payment/failure';
				break;
			case -1:
				//交易待处理
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_pending'),
				Mage::helper('opapplepay')->__(self::BrowserReturn.'(Pre-auth)Payment Pending!'.$history));
				...
				if($_REQUEST['payment_authType'] == 1){
					...
					$url = 'opapplepay/payment/pending';
				}
				break;
			case '10000':
				//10000:Payment is declined 高风险订单
				$order->addStatusToHistory(
				$model->getConfigData('order_status_payment_risk'),
				Mage::helper('opapplepay')->__(self::BrowserReturn.'(High Risk)Payment Failed!'.$history));
	...
	public function successAction()
	{
		...
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opapplepay/success'));
		$this->renderLayout();
	}
	...
	public function failureAction()
	{
		...
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opapplepay/failure'));
		$this->renderLayout();
	}
	...
	public function pendingAction()
	{
		$this->loadLayout();
		$this->getLayout()->getBlock('root')->setTemplate('page/2columns-right.phtml');
		$this->getLayout()->getBlock('content')->append($this->getLayout()->createBlock('opapplepay/pending'));
		$this->renderLayout();
	}
	...
	{
		$session = Mage::getSingleton('checkout/session');
		$errorMsg = Mage::helper('opapplepay')->__(' There was an error occurred during paying process.');
		...
		if ($order instanceof Mage_Sales_Model_Order && $order->getId()) {
			$order->addStatusToHistory(
					...
					Mage::helper('opapplepay')->__('Customer returned from ApplePay.') . $errorMsg
			);
	...
	public function loadingGif()
	{
		...
		echo '<img src="'.Mage::getDesign()->getSkinUrl('images/op_applepay/loading.gif').'" />';
